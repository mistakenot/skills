...
- `src/compile.py:305-344` — `__main__`: modules are **hard-coded** (no glob/auto-discovery) and passed to `compile([...])`. Adding `src/assurance/` requires a new `module("assurance", ...)` declaration appended to that list. `install.sh` is regenerated from this same list (subagent-reported, lines ~211-298), so the new module auto-registers there.
...
- **`make` targets are thin wrappers**: `compile:` → `python3 src/compile.py`; `lint:` → `autoskill lint`; `check:` → compile + lint. A new `eval-assurance` target follows the same one-line-wrapper pattern.
...
## Git History & Current State (verified 2026-06-13)

- **`8ea4266`** "Add task 001 assurance-eval groundwork; fix auto doc/skill tooling rename" (HEAD) added this task's docs (requirements + research diary + spike + headless-cli reference) and renamed tooling CLIs. **It added docs only — no `src/` code.** Confirmed `ls src/assurance` → *No such file or directory*. This task is the first to write `src/assurance/`.
