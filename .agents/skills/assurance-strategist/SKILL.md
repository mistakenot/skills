...
| Differential Testing | Runs a second independent implementation — a deliberately-simple reference oracle, or an external spec/competitor/legacy build — on the same inputs and asserts identical outputs, manufacturing an oracle where none exists | differential | algorithmic-core | C2 | both | [Differential Testing](references/technique-differential-testing.md) |
| Metamorphic Testing | Asserts known relations between the outputs of related inputs when no oracle exists — transforms an input in a known way and checks the output changes as it must; catches inconsistency, not wrongness | metamorphic | algorithmic-core | C2 | both | [Metamorphic Testing](references/technique-metamorphic-testing.md) |
