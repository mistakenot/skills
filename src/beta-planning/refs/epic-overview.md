...
This is the `epic.html` produced by `{{ skill:beta-new-epic }}`, one altitude above the task-level `plan.html`.
...
- Each breakdown item becomes a task, planned later with `{{ skill:beta-new-task }}`.
