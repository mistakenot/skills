...
[/{{ skill:status-report }}](../status-report/SKILL.md) — operates on the
...
`{{ skill:new-task }}`, `{{ skill:new-solution }}`, `{{ skill:new-plan }}`,
`{{ skill:review-task }}` and `{{ skill:commit-task }}` run in a planner.

**Workers** execute tasks in isolated worktrees and open PRs.
`{{ skill:execute-task }}` runs in a worker. **Workers must never work in the
