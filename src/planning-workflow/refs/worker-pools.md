...
[/{{ skill:delegate }}](../delegate/SKILL.md),
[/{{ skill:delegate-task }}](../delegate-task/SKILL.md), and
[/{{ skill:status-report }}](../status-report/SKILL.md) — carries the `runner`
...
planning, review, and commit skills (`{{ skill:new-task }}`,
`{{ skill:new-solution }}`, `{{ skill:new-plan }}`, `{{ skill:review-task }}`,
`{{ skill:commit-task }}`) run in a planner pane.

**Workers** execute tasks in isolated worktrees with their own branch and
tooling, and open PRs when complete. `{{ skill:execute-task }}` runs in a
...
surplus idle panes in `{{ skill:status-report }}`) is documented in
`{{ skill:status-report }}` itself; the reap mechanism per runner is `references/{{ .runner }}/reap-worker.md`.
