...
delegate family — [/{{ skill:delegate }}](../delegate/SKILL.md),
[/{{ skill:delegate-task }}](../delegate-task/SKILL.md), and
[/{{ skill:status-report }}](../status-report/SKILL.md) — dispatches into the
...
tooling, and open PRs when complete. `{{ skill:execute-task }}` runs in a worker
...
below 4, never exceed 6, reclaim surplus idle panes in `{{ skill:status-report }}`)
is documented in
