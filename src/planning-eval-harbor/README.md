...
| Term | Meaning |
|---|---|
| **Fixture** | The planning-eval fixture JSON, unchanged: `fixture.target_repo`, an immutable `start_sha`, the opening `prompt`, `human_turns`, the `arm`, `limits`. Two optional keys this harness reads: `limits.max_budget_usd` (hard spend cap, passed to Claude Code) and `persona` (simulated operator only). See planning-eval's README for how to mine one. |
| **Arm** | `arm.skills_dir`, injected into the sandbox by Harbor (`agents[].skills`) and registered in the agent's `CLAUDE_CONFIG_DIR/skills`. The target repo's own `.claude/skills/` is stripped from the seed tree, so — as in planning-eval — two arms differ only by workflow. Harbor records every skill's content digest in `lock.json`. |
